...
df_original = pd.read_csv ('G:/My Drive/ExportVisual3d.txt', delimiter='\t')
df_original = df_original.iloc[0:4]

#ammended import for needs
df = pd.read_csv ('G:/My Drive/ExportVisual3d.txt', delimiter='\t', skiprows=[1,2,3,4], header=0)
...
import os
import os.path
file_list = os.listdir("G:/My Drive/Adam_G_Thesis/4_Data/3. Pilot/Raw_data/Raw_EMG_Data/2021-02-06_TAFTPilot/C#D")
"""

#*** makes list out of dataframe names
file_list = list(df.columns.values)
file_list.remove("Time")
...
#rename first header to NaN
df_original.rename(columns={"Unnamed: 0" : np.nan}, inplace= True)
